  ...
  sendDataUpdate(data) {
    try {
      ...
      if (!chrome.runtime || !chrome.runtime.sendMessage) {
        this.addDebugInfo(`🔌 Extension context unavailable - stopping monitor`, 'warning');
        this.stop();
        return;
      }
      ...
      chrome.runtime.sendMessage({
        ...
        if (error.message.includes('Extension context invalidated') || 
          ...
          this.addDebugInfo(`🔄 Extension context invalidated - stopping monitor`, 'warning');
          this.stop();
        } else {
          this.addDebugInfo(`⚠️ Failed to send data update: ${error.message}`, 'warning');
        }
        // Always ignore messaging errors in optimized version
      });
    } catch (error) {
      if (error.message.includes('Extension context invalidated') || 
          error.message.includes('context invalidated') ||
          error.message.includes('The extension context')) {
        this.addDebugInfo(`🔄 Extension context invalidated in try block - stopping monitor`, 'warning');
        this.stop();
      } else {
        this.addDebugInfo(`❌ Error in sendDataUpdate: ${error.message}`, 'error');
      }
      // Always ignore messaging errors
    }
  }
